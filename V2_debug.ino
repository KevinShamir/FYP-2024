...
int rxPin = 2;
int txPin = 3;
int homeSwitchCarrossel = 4; //rotating table limit switch
int homeSwitchLongitudinal = 22; //longitudinal arm limit switch
int homeSwitchLateral = 23; //Lateral arm limit switch
int homeSwitchVertical = 24; //Vertical arm limit switch

const int pul1 = 5; //Motor 1 Carrossel 200 microsteps and 1.5-1.7 amps
const int dir1 = 6;

const int pul2 = 7; //Motor 2 Longitudinal (Front and Back)
const int dir2 = 8;

const int pul3 = 9; //Motor 3 Lateral (Left and Right)
const int dir3 = 10;
...
const int forceClose=90; //Used to force close the gripper
const int gripperOpen=60;
...
const int fan = 50; // steps to rotate to fan from input area 90 degrees
const int pickup = 50; //steps to rotate from fan to pickup area 90 degrees
const int lateralPickup = 20; //Steps to move lateral base from LIMIT SWITCH LATERAL to PICKUP POINT      //original 2000
const int verticalPickup = 20; //Steps to move vertical base from LIMIT SWITCH LATERAL to PICKUP POINT
const int longitudinalPickup = 20; //Steps to move the longitudinal base from LIMIT SWITCH LATERAL to magazine
const int longitudinalScanIntermediate = 10; //Steps to move the longitudinal base from magazine to an intermediate pickup, before QR Scanning
const int longitudinalScan = 10; // Steps to move the longitudinal base from intermediate to scanning area (Both barcode and U.S)
const int lateralScan = 10;  //Steps to move the lateral base from pickup to QR Scanning
const int verticalScan = 50; //Steps to move the vertical base from pickup to QR Scanning

//MotorSpeed initialisation
const int carroSpeed = 400000; //delay value in microseconds
const int lateralSpeed = 400000; //delay value in microseconds
const int verticalSpeed = 400000; //delay value in microseconds
const int longitudinalSpeed = 400000; //delay value in microseconds

//U.S Variables
long duration;
int distance;
int distanceUsToSlide = 10; //minimum distance recorded if glass slide is placed under U.S
long distanceScan; // Used to store distance value

//Others
const int fanTiming = 1000; //1 seconds fan duration, in milli seconds
...
const int glassSlideStepInput = 50; //Number of motor steps between each glass slide in the magazine input
const int scanDuration = 5000; //max period of 5 seconds for barcode scanner to read QR Code once it is placed under the scanner
...
int magazine_num;
int magazinecount; //global count of number of magazines used
...
// Function to reset the Carrossel to limit switch location
void carrosselReset(){
  ...
  while(state){
    ...
    delayMicroseconds(400);
   if(digitalRead(homeSwitchCarrossel)==HIGH){
    state = 0; //stop moving once switch is hit
    digitalWrite(dir1,LOW); //Set motor rotating direction to clockwise default
...
void verticalReset(){
  ...
  while(state){
   ...
   if(digitalRead(homeSwitchVertical)==HIGH){
    state = 0; //stop moving once switch is hit
    digitalWrite(dir4,LOW); //Set motor rotating direction to clockwise default
    //digitalWrite(dirPin,HIGH);
   }
      ...
      while(1){
        if(Serial.available()>0){
          ...
          if(magazine_num>0 && magazine_num == int(magazine_num)){
            ...
            //carrosselReset(); //Reset input magazine towards limit switch. No check with laser sensor needed as check is done with limit switch
            Serial.println("Received"); // Send Received message to Raspi
            ...
            while(1){
              ...
              if(Serial.available()>0){
                ...
                if(message.equals("Inserted")){
                  ...
                  motorStep(fan,pul1,dir1,0,carroSpeed); //rotate motor by 90 degrees-clockwise
                  ...
                  motorStep(pickup,pul1,dir1,0,carroSpeed); //rotate motor by 90 degrees-clockwise
                  ...
                  //Serial.println("I am at the Interm point"); //debug
                  ...
                  while(distanceScan>distanceUsToSlide){
                    ...
                    while(condition){
                      ...
                      if(elapsed < scanDuration TO change){ //elapsed < scanDuration TO change
                    ...
                    //Serial.println("I am at the output magazine slot");
                    ...
                    if(magazine_num==2 && magazineCount<3){
              ...
              }//Serial.print("hi"); add lines of code after break
            }
            break; //break out of the while loop for magazine_num
            //Serial.println("I am breaking out of magazine_num while loop"); //debug
       
      Serial.print()""Finished; //senmessage sent to ardRaspi to indicate a completed ,magazine